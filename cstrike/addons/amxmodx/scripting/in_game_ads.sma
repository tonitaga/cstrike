//================================
// Название: In Game Advertisement System (MastaMan Edition)
// Версия: 1.83
...
#define CONFIG_FOLDERNAME "in_game_ads"
#define MAIN_PRECACHE_FILE "in_game_ads.cfg"
...
public plugin_init()
{
	register_plugin("In-Game Ads(MastaMan Edition)", "1.83 (Edition 1.1.0)", "stupok")
	register_dictionary("in_game_ads.txt")
	...
	register_clcmd("iga_scale_up", "cmd_scale_sprite_up", ADMIN_ACCESS_LEVEL)
	register_clcmd("iga_scale_down", "cmd_scale_sprite_down", ADMIN_ACCESS_LEVEL)
...
public plugin_precache()
{
	...
	{
		log_amx("%L", LANG_SERVER, "ERROR_FILE_NOT_FOUND", DELETE_ICON_PATH)
	}
...
public cmd_place_ad(id)
{
	...
	switch(cmd[0])
	{
		case '+':
		{
			...
			{
				log_amx("%L", LANG_SERVER, "ERROR_USER_ALREADY_PLACING")
				client_print(0, print_chat, "%L", LANG_SERVER, "ERROR_USER_ALREADY_PLACING")
				return PLUGIN_HANDLED
			}
...
public cmd_scale_sprite_down(id)
{
	if(!(get_user_flags(id) & ADMIN_ACCESS_LEVEL))
	{
		client_print(id, print_console, "You do not have access to this command.")
...
{	
	...
	new nLen = formatex(szMenuBody, MENU_SELECT_SIZE-1, "%L", LANG_SERVER, "MENU_SELECT_TEXT_CHOOSE_MODEL", page+1, (g_precached_model_list_len / MENU_SELECT_OPTIONS_NUM + ((g_precached_model_list_len % MENU_SELECT_OPTIONS_NUM) ? 1 : 0 )) )
	...
	{
		nKeys |= (1<<nCurrKey++)
		nLen += formatex( szMenuBody[nLen], (MENU_SELECT_SIZE-1-nLen), "\r%i.\w %s^n", nCurrKey, g_precached_model_list[i] )
	}
	
	new szAxis[40]
	add_to_rotate(g_ent, -1)
		
	switch(g_iAxis)
	{
		case 0: formatex(szAxis, sizeof(szAxis) - 1, "\r[X] \dY \dZ \y(%.0f%L)\w", f_Angles[0], LANG_SERVER, "DEG")
		case 1: formatex(szAxis, sizeof(szAxis) - 1, "\dX \dY \r[Z] \y(%.0f%L)\w", f_Angles[1], LANG_SERVER, "DEG")
		case 2: formatex(szAxis, sizeof(szAxis) - 1, "\dX \r[Y] \dZ \y(%.0f%L)\w", f_Angles[2], LANG_SERVER, "DEG")
	}
	
	if( nEnd != g_precached_model_list_len )
	{
		formatex( szMenuBody[nLen], (MENU_SELECT_SIZE-1-nLen), "%L", LANG_SERVER, "MENU_SELECT_TEXT_OPTIONS_1", szAxis, ROTATE_ANGLE, LANG_SERVER ,page ? "MENU_TEXT_PREVIOUS" : "MENU_TEXT_EXIT" )
		nKeys |= (1<<8)|(1<<7)|(1<<6)|(1<<5)|(1<<4)
	}
	else
	{
		formatex( szMenuBody[nLen], (MENU_SELECT_SIZE-1-nLen), "%L", LANG_SERVER, "MENU_SELECT_TEXT_OPTIONS_2", szAxis, ROTATE_ANGLE, LANG_SERVER, page ? "MENU_TEXT_PREVIOUS" : "MENU_TEXT_EXIT" )
...
{
	new tmpML[128]
	formatex(tmpML, sizeof(tmpML) - 1, "%L", LANG_SERVER, "MENU_SAVE_TEXT")
	show_menu(id, (1<<0)|(1<<9), tmpML, -1, "menu_save")
}
...
public Showmenu_delete(id, page)
{
	...
	new nLen = formatex(szMenuBody, MENU_DELETE_SIZE-1, "%L", LANG_SERVER, "MENU_DELETE_TEXT_HEADER", page+1, (g_map_model_list_len / MENU_DELETE_OPTIONS_NUM + ((g_map_model_list_len % MENU_DELETE_OPTIONS_NUM) ? 1 : 0 )) )
	...
	{
		nKeys |= (1<<nCurrKey++)
		nLen += formatex( szMenuBody[nLen], (MENU_DELETE_SIZE-1-nLen), "\r%i.\w %s^n", nCurrKey, g_map_model_list[i] )
	}
	
	if( nEnd != g_map_model_list_len )
	{
		formatex( szMenuBody[nLen], (MENU_DELETE_SIZE-1-nLen), "%L", LANG_SERVER, "MENU_DELETE_TEXT_OPTIONS_1", LANG_SERVER, page ? "MENU_TEXT_PREVIOUS" : "MENU_TEXT_EXIT" )
		nKeys |= (1<<8)
	}
	else
	{
		formatex( szMenuBody[nLen], (MENU_DELETE_SIZE-1-nLen), "%L", LANG_SERVER, "MENU_DELETE_TEXT_OPTIONS_2", LANG_SERVER, page ? "MENU_TEXT_PREVIOUS" : "MENU_TEXT_EXIT" )
		nKeys |= (1<<7)|(1<<8)
	}
...
{
	new tmpML[128]
	formatex(tmpML, sizeof(tmpML) - 1, "%L", LANG_SERVER, "MENU_DELETE_TEXT_CONFIRM")
	show_menu(id, (1<<0)|(1<<9), tmpML, -1, "menu_confirm")
}
			...
			if(dir_exists(file_text))
			{
				...
				while(next_file(dirh, file_text, MAX_MODEL_NAMELEN - 1))
				{
					if(contain(file_text, ".spr") != -1 || contain(file_text, ".mdl") != -1)
					{
						...
						{
							log_amx("%L", LANG_SERVER, "ERROR_FILE_NOT_FOUND", file_text)
						}
						else
						{
...
stock add_to_rotate(entid, g_iAxis)
{
	if(!pev_valid(entid))
		return 0
		
	pev(entid, pev_angles, f_Angles)
	
	if(g_iAxis != -1)
	{
		f_Angles[g_iAxis] += ROTATE_ANGLE

		if(f_Angles[g_iAxis] > 360.0)
			f_Angles[g_iAxis] = 0.0
	
		set_pev(entid, pev_angles, f_Angles)
	}
